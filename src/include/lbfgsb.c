...
//       double *Fmin, optimgrC fminfn, int *fail, double factr, double pgtol,
...
// 	//if (nREPORT <= 0)  error(_("REPORT must be > 0 (method = \"L-BFGS-B\")"));
// 	switch(trace) {
// 		case 2: tr = 0; break;
...
// 			f = fminfn(n, x, g);
// 			//if (!R_FINITE(f))    error(_("L-BFGS-B needs finite values of 'fn'"));
// 			//fmingr(n, x, g, ex);
...
typedef double (*optfn)(int, double *, double *);





void lbfgsb(int n, double *x, double *l, double *u,
      double *Fmin, optfn fminfn, int maxit)
{
    char task[60];
    double *g,  *wa;
    int iter = 0, *iwa, isave[21], *nbd, m=5;
    isave[12] = 0; // -Wall
	...
	for(int i = 0; i < n; i++) nbd[i] = 2;
	strcpy(task, "START");
	while(iter < maxit) {
		setulb(n, m, x, l, u, nbd, Fmin, g, factr, &pgtol, wa,
			 iwa, task, -1, isave);
		if (strncmp(task, "FG", 2) == 0) *Fmin = fminfn(n, x, g);
		else if (strncmp(task, "NEW_X", 5) == 0) iter++;
		else break;
...
double branin(int n, double xx[2], double grad[2]) {
  ...
  double d = x2 - b *x1*x1 + c * x1 - r;
  double term1 = a * d*d;
  double term2 = s * (1 - t) * cos(x1);
  grad[0] = 2*a*d *(c - 2*b*x1) - s * (1-t)*sin(x1);
  grad[1] = 2*a*d;
...
int main(){
	...
	double x[2];
	double l[n] = {-5., 0.}, u[n] = {10., 15};
	double Fmin;

	int maxit = 100, trace= -1;
	lbfgsb(n, x, l, u, &Fmin, branin, maxit);
